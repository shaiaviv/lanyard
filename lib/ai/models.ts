...
/** The Whisper transcription model bound to the configured OpenAI key. */
export async function whisperModel() {
  const apiKey = await getServiceKey('openai');
  if (!apiKey) throw new MissingServiceKeyError('openai');
  return createOpenAI({ apiKey }).transcription(MODELS.WHISPER);
}
